...
import { UnauthorizedUserError, UserAlreadyExistsError, UserIsAdminError, UserNotFoundError, InvalidBirhdateError } from "../errors/userError";
...
class UserDAO {
    ...
    getAllUsers(): Promise<User[]> {
        return new Promise<User[]>((resolve, reject) => {
                ...
                db.all(sql, (err: Error | null, rows: any) => {
                    ...
                    const users: User[] = rows.map( (row: { username: string; name: string; surname: string; role: Role; address: string; birthdate: string; })  => { 
    ...
    deleteUser(user: User, username: string): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
                ...
                db.get(sql, [username], (err: Error | null, row: { role: Role }) => {
                    if (err) {
                        reject(err);
                        return;
                    }
                    ...
                    if (user.role === Role.ADMIN && row.role === Role.ADMIN && user.username !== username) {
                        reject(new UserIsAdminError());
                        return;
                    }
    
                    const deleteSql = "DELETE FROM users WHERE username = ?";
                    db.run(deleteSql, [username], (err: Error | null) => {
                        if (err) {
                            reject(err);
                            return;
                        }
    ...
    updateUserInfo(user: User, name: string, surname: string, address: string, birthdate: string, username: string): Promise<User> {
        return new Promise<User> ((resolve, reject) => {
                ...
                if (birthdateObj >= new Date()){
                    reject(new InvalidBirhdateError());
                    return;
                }
                if(user.username !== username && user.role != Role.ADMIN){
                    reject(new UnauthorizedUserError());
                ...
                db.get(sql, [username], (err: Error | null, row: any) => {
                    ...
                    db.run(updateSql, [name, surname, address, birthdate, username], (err: Error | null) => {
                        if(err) {
                            reject(err);
                            return;
                        }
                        resolve(new User(username, name, surname, row.role, address, birthdate));
                    });
                });
        });
