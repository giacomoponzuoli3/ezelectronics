    ...
    initRoutes() {
        ...
        this.router.get(
            ...
            (req: any, res: any, next: any) => {
                ...
                if(loggedUser.role === Role.ADMIN || loggedUser.username === requestedUsername) {
                    this.controller.getUserByUsername(loggedUser, requestedUsername)
                    .then((user: User) => res.status(200).json(user))
                    .catch((err) => next(err));
                }
                else {
                    return res.status(401).json({  error: new UnauthorizedUserError() })
                }
